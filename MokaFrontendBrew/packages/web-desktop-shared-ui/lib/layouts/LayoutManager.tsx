...
const TopBarLayout: React.FC<{ header?: ReactNode, children: ReactNode }> = ({ header, children }) => (
    <div className="flex flex-col h-screen">
        <header className="bg-gray-800 text-white p-4">{isValidElement(header) ? header : <DefaultHeader />}</header>
        <main className="flex-1 overflow-auto">{children}</main>
    </div>
);

const SideBarLayout: React.FC<{ header?: ReactNode, children: ReactNode }> = ({ header, children }) => (
...
const DefaultHeader: React.FC = () => <div className="flex justify-between items-center h-12">
