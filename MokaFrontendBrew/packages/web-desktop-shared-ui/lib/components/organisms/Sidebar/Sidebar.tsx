...
const Sidebar: React.FC<SidebarProps> = ({ items, showAvatar, avatarUrl, avatarLabel, onAvatarClick }) => {
    return (
        <aside className={sidebarStyles.base}>
            <div className={`${showAvatar ? 'pt-8' : 'pt-4'} flex flex-col items-center w-full`}>
                {showAvatar && avatarUrl && (
                    <div className="flex flex-col items-center mb-4">
                        <button onClick={onAvatarClick} className="mb-2">
                            <img src={avatarUrl} alt="Avatar" className="w-12 h-12 rounded-full object-cover shadow-sm" />
                        </button>
