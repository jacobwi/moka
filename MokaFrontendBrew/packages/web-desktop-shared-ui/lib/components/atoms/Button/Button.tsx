...
const Button: React.FC<ButtonProps> = ({
  ...
  const iconClasses = Object.keys(rest)
    ...
    .map((key) => {
      ...
      const iconStyleVariant =
        buttonStyles.props.icon[iconProp as keyof typeof buttonStyles.props.icon];
      if (isObject(iconStyleVariant) && isObject(iconValue) && iconValue in iconStyleVariant) {
  ...
  const buttonClasses = twMerge(buttonStyles.base, dynamicClassNames, className); // Add dynamicClassNames if needed

  return (
    <button className={buttonClasses} {...rest}>
      {iconPosition === 'left' && iconName && (
        <IconWrapper iconName={iconName} {...iconProps} className={iconClasses} />
      )}
      {children}
